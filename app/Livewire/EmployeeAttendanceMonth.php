...
trait EmployeeAttendanceMonth
{
    ...
    private function employeeMonthData(): array
    {
        ...
        $employees = User::query()->whereNotIn('role', ['member', 'admin', 'head_coach'])
            ->where('is_active', true)
            ->where('is_sales_online', false)
            ...
            ->reject(fn (User $user): bool => $user->isHeadCoach());
        ...
        $totals = $employees->mapWithKeys(function (User $user): array {
            $counts = $user->employeeAttendances->countBy(fn ($attendance) => $attendance->status->value);

            return [$user->id => [
                'hadir' => $counts->get('hadir', 0),
                'off' => $counts->get('off', 0),
                'izin' => $counts->get('izin', 0) + $counts->get('sakit', 0),
                'all' => $user->employeeAttendances->count(),
            ]];
        });
