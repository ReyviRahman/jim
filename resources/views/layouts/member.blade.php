...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body @class(["member-pt-page" => request()->routeIs("member.jadwal-pt.*"), "member-attendance-page" => request()->routeIs("member.kehadiran.*")]) @if(request()->routeIs('member.kehadiran.*')) style="--attendance-background: url('{{ asset('member-attendance-gym-v2.png') }}')" @endif>
        ...
        <div class="gym-sidebar flex h-full flex-col overflow-y-auto [&>*]:shrink-0">
            <x-sidebar-brand />
            <x-sidebar-category>MEMBER</x-sidebar-category>
            <ul class="gym-sidebar-menu">
                <li>
                    <x-sidebar-item :href="route('member.dashboard')" :active="request()->routeIs('member.dashboard')" title="Dashboard" description="Ringkasan aktivitas gym">
                ...
                <li>
                    <x-sidebar-item :href="route('member.absensi')" :active="request()->routeIs('member.absensi')" title="Absensi" description="Scan QR / check-in gym">
                ...
                <li>
                    <x-sidebar-item :href="route('member.kehadiran.index')" :active="request()->routeIs('member.kehadiran.*')" title="Kehadiran" description="Riwayat kehadiran Anda">
                ...
                @if ($hasPtMembership)
                    <li>
                        <x-sidebar-item :href="route('member.jadwal-pt.index')" :active="request()->routeIs('member.jadwal-pt.*')" title="Jadwal PT" description="Booking sesi personal training">
                ...
                @endif
                {{-- <li>
                    <x-sidebar-item :href="route('member.paket.index')" :active="request()->routeIs('member.paket.*')" title="Paket GYM" description="Pilihan membership gym">
