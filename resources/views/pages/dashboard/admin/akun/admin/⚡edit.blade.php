...
new #[Layout('layouts::admin')] class extends Component 
{
    ...
    public $userId; // Menyimpan ID user yang sedang diedit
    public $name = '';
    public $age = '';
    ...
    public $photo = null; // Untuk upload foto baru
    public $existingPhoto = null; // Untuk menampilkan foto lama
    ...
    // Aturan validasi dinamis (Penting untuk mengecualikan ID saat cek unique)
    protected function rules()
    {
        return [
            ...
            'role' => 'required|in:admin,kasir_gym,kasir_minum',
            'shift' => 'nullable|in:Pagi,Siang,Full',
            'email' => 'required|email|unique:users,email,' . $this->userId,
            'password' => 'nullable|min:6', // Nullable agar tidak wajib ganti password
            'photo' => 'nullable|image|max:10048', // Nullable agar tidak wajib ganti foto
        ];
    }
    ...
    public function update() 
    {
        ...
        // Logika untuk menyimpan foto baru (dan hapus yang lama jika ada)
        if ($this->photo) {
            if ($user->photo) {
        ...
        $user->role = $this->role;
        $user->shift = $this->role === 'kasir_gym' ? $this->shift : null;
...
<div>
    <h1 class="text-3xl text-center font-semibold mb-6">Edit Akun Pegawai</h1>
    
    <form wire:submit.prevent="update">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            
            <div class="mb-4 sm:col-span-2">
                <label for="photo" class="block mb-2.5 text-sm font-medium text-heading">Foto Profil Pegawai (Opsional)</label>
                
                {{-- Menampilkan foto baru jika ada, kalau tidak ada tampilkan foto lama --}}
                @if ($photo)
                    <div class="mb-3">
                ...
                @elseif ($existingPhoto)
                    <div class="mb-3">
                        <img src="{{ asset('storage/' . $existingPhoto) }}" class="w-20 h-20 object-cover rounded-full border border-gray-300">
                    </div>
                @endif
                ...
                <p class="text-xs text-gray-500 mt-1">Biarkan kosong jika tidak ingin mengubah foto profil.</p>
            ...
            <div class="mb-4">
                <label for="email" class="block mb-2.5 text-sm font-medium text-heading">Email Login</label>
            ...
            <div x-data="{ show: false }">
                <label for="password" class="block mb-2.5 text-sm font-medium text-heading">Password (Opsional)</label>
                
                <div class="relative">
                    {{-- Hapus atribut required di input ini --}}
