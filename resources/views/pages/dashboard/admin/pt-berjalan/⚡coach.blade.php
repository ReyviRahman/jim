...
new #[Layout('layouts::admin')] class extends Component
{
    ...
    {
        return $this->coachId === null ? 'Belum ada coach' : User::findOrFail($this->coachId)->name;
    }

    #[Computed]
    public function packageCount(): int
    {
        return $this->membershipQuery()->count();
    }

    #[Computed]
    public function coachMembership(): ?Membership
    {
        return $this->selectedMembershipForCoach === null ? null : $this->membershipQuery()->with(['user', 'ptPackage'])->find($this->selectedMembershipForCoach);
    }

    public $showCoachModal = false;
    #[Locked]
    public ?int $selectedMembershipForCoach = null;
    ...
    public function openCoachModal(int $membershipId): void
    {
        abort_if(auth()->user()->isHeadCoach(), 403);
        $membership = ($this->membershipQuery()->find($membershipId) ?? abort(404));
        abort_if($membership->pt_id !== null, 403);
    ...
    {
        ($this->membershipQuery()->find($membershipId) ?? abort(404));
    ...
    public function openCoachModalFromDetail(int $membershipId): void
    {
    ...
    public function selectedMembership(): ?Membership
    {
        ...
        return $this->membershipQuery()->with(['user', 'members', 'admin', 'followUp', 'followUpTwo', 'personalTrainer', 'ptPackage'])
            ->withMin(['ptBookings as first_approved_booking_date' => fn (Builder $query) => $query->where('status', 'approved')], 'booking_date')
            ->withCasts(['first_approved_booking_date' => 'date'])
    ...
    {
        abort_if(auth()->user()->isHeadCoach(), 403);
        $this->validate(['selectedCoachId' => ['required', 'integer', Rule::exists('users', 'id')->where('role', 'pt')->where('is_active', true)]], [
            'selectedCoachId.required' => 'Pilih coach terlebih dahulu.',
            'selectedCoachId.exists' => 'Pilih coach PT yang aktif.',
        ]);
        $membership = ($this->membershipQuery()->whereNull('pt_id')->find($this->selectedMembershipForCoach) ?? abort(404));
        $membership->update(['pt_id' => $this->selectedCoachId]);
        session()->flash('success', 'Coach berhasil dipilih!');
        $this->closeCoachModal();
        $this->closeDetailModal();
        $this->resetPage();
        unset($this->memberships, $this->packageCount, $this->selectedMembership);
    }
    ...
    {
        $query = $this->membershipQuery()->with(['user', 'members', 'admin', 'followUp', 'followUpTwo', 'personalTrainer', 'ptPackage']);
        ...
        return $query->orderByRaw('COALESCE(remaining_sessions, 999) ASC')->orderBy('id')->paginate(12);
    }
};
?>
...
<div>
    ...
    @if ($showCoachModal && $selectedMembershipForCoach)
        @php
            $coachMembership = $this->coachMembership;
        @endphp
