...
new #[Layout('layouts::admin')] class extends Component
{
    ...
    public function coaches(): LengthAwarePaginator
    {
        return User::query()->where('role', 'pt')
            ->where(fn (Builder $query) => $query->where('is_active', true)
                ->orWhereHas('ptMemberships', fn (Builder $memberships) => $memberships->runningPt()))
    ...
    {
        return Membership::query()->runningPt()->whereNull('pt_id')->count();
    }
};
?>
...
<div class="space-y-6">
    <div>
        <h1 class="text-xl font-semibold text-heading">Data PT Berjalan</h1>
    ...
    @if ($this->unassignedCount > 0)
        <a href="{{ route('admin.pt-berjalan.unassigned') }}" wire:navigate class="flex items-center justify-between gap-3 rounded-lg border border-orange-200 bg-orange-50 p-4 text-orange-800 focus-visible:ring-2 focus-visible:ring-orange-500">
            <h2 class="min-w-0 break-words font-semibold">Belum ada coach</h2>
            <span class="shrink-0 text-sm font-medium">{{ $this->unassignedCount }} member aktif</span>
        </a>
    @endif

    <div class="grid grid-cols-1 gap-3">
        @forelse ($this->coaches as $coach)
            <a wire:key="coach-{{ $coach->id }}" href="{{ route('admin.pt-berjalan.coach', ['coach' => $coach->id]) }}" wire:navigate class="flex items-center justify-between gap-3 rounded-lg border border-default bg-white p-4 shadow-xs transition-shadow hover:shadow-md focus-visible:ring-2 focus-visible:ring-brand">
                ...
                <span class="shrink-0 text-sm font-medium text-heading">{{ $coach->active_packages_count }} member aktif</span>
            </a>
        @empty
